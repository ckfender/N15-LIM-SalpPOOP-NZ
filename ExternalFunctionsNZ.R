...
ResetRN15 <- function(Aa,flows,wts,d15N,del15NMeso,del15NMacro,del15NSalp){
  ...
  RGelfood <- (RMeso*flows[MesoTOgel] + RVMMeso*flows[VMMesoTOgel] + RMacro*flows[MacroTOgel] + 
                 RVMMacro*flows[VMMacroTOgel])/Gelcons
  RSalpfood <- (RPico*flows[PicoTOSalp] + RDtm*flows[dtmTOSalp] + RFlag*flows[flagTOSalp] + RHNF*flows[hnfTOSalp] +
    RMic*flows[micTOSalp])/Salpcons
  ...
  RMyctfood <- (RMeso*flows[MesoTOMyct] + RVMMeso*flows[VMMesoTOMyct] + RMacro*flows[MacroTOMyct] + RVMMacro*flows[VMMacroTOMyct] +
                  RSalp*flows[SalpTOMyct] + RAmph*flows[AmphTOMyct])/Myctcons
  
 #Computing the sum of each flow for normalization purposes
  totflowNO3 <- sum(flows[c(Upwelling,NH4toNO3)])
  totflowNH4 <- sum(flows[c(bacTOnh4,hnfTOnh4,micTOnh4,MesoTOnh4,VMMesoTOnh4,MacroTOnh4,VMMacroTOnh4,gelTOnh4,MyctTOnh4,SalpTOnh4,AmphTOnh4)])
  ...
  totflowMeso <- sum(flows[c(dtmTOMeso,flagTOMeso,hnfTOMeso,MesoTOMeso,VMMesoTOVMMeso,mdetTOMeso)])
  totflowVMMeso <- sum(flows[c(dtmTOVMMeso,flagTOVMMeso,hnfTOVMMeso,MesoTOVMMeso,VMMesoTOVMMeso,mdetTOVMMeso)])
  totflowMacro <- sum(flows[c(dtmTOMacro,flagTOMacro,hnfTOMacro,micTOMacro,MesoTOMacro,VMMesoTOMacro)])
  totflowVMMacro <- sum(flows[c(dtmTOVMMacro,flagTOVMMacro,hnfTOVMMacro,micTOVMMacro,MesoTOVMMacro,VMMesoTOVMMacro)])
  totflowGel <- sum(flows[c(MesoTOgel,VMMesoTOgel,MacroTOgel,VMMacroTOgel)])
  ...
  totflowMyct <- sum(flows[c(MesoTOMyct,VMMesoTOMyct,MacroTOMyct,VMMacroTOMyct,SalpTOMyct,AmphTOMyct)])
  totflowBac <- sum(flows[c(domTObac)])
  totflowSdet <- sum(flows[c(PicoTOsdet,flagTOsdet,hnfTOsdet,micTOsdet)])
  totflowMdet <- sum(flows[c(dtmTOmdet,MesoTOmdet,VMMesoTOmdet)])
  totflowLdet <- sum(flows[c(MacroTOldet,VMMacroTOldet,gelTOldet,AmphTOldet)])
  totflowSalpDet <- sum(flows[c(SalpTOSalpDet)])
  totflowDOM <- sum(flows[c(PicoTOdom,dtmTOdom,flagTOdom,hnfTOdom,micTOdom,MesoTOdom,VMMesoTOdom,MacroTOdom,VMMacroTOdom,gelTOdom,MyctTOdom,SalpTOdom,AmphTOdom,sdetTOdom,mdetTOdom,ldetTOdom,SalpDetTOdom)])
