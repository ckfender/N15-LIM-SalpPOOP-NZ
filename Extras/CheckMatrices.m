...
%%Switch to turn on plotting. I highly recommend you comment out or modify
%%some of the graphs of flows or this may take a very long time.
plot=0;
...
%%Adding two columns to compare 50th percentiles minus the 20% burn in,
%%informative to convergence
len = length(AaOut);
temp = AaOut(:,round(len*0.2):len);
cut = length(temp)/2;
AaOutmean(:,1)=mean(temp,2);
...
RMSEall=sqrt(mean((abs(AaOutmean(:,1)-ba)).^2));
if Cycle==1
    RMSErate=sqrt(mean((abs(AaOutmean(1:19,1)-ba(1:19))).^2));
else
    RMSErate=sqrt(mean((abs(AaOutmean(1:20,1)-ba(1:20))).^2));
end
