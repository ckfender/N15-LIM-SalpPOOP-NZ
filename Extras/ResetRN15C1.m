function [output] = ResetRN15C1(Aa,flows,wts,d15N,del15NMeso,del15NMacro,del15NSalp)
%This code modifies the 15N portion of the Aa matrix to contain it's actual
%modelled values (Rmes is NaN in Aa0, this replaces with actual modelled
%Rmes) divided by the total flow of N through the compartment and corrected for the weights. 
...
totflowNH4 =  sum(flows([bacTOnh4,hnfTOnh4,micTOnh4,MesoTOnh4,MacroTOnh4,VMMacroTOnh4,gelTOnh4,MyctTOnh4,SalpTOnh4,AmphTOnh4]));
...
totflowMeso =  sum(flows([dtmTOMeso,flagTOMeso,hnfTOMeso,MesoTOMeso,mdetTOMeso]));
totflowMacro =  sum(flows([dtmTOMacro,flagTOMacro,hnfTOMacro,micTOMacro,MesoTOMacro]));
totflowVMMacro =  sum(flows([dtmTOVMMacro,flagTOVMMacro,hnfTOVMMacro,micTOVMMacro,MesoTOVMMacro]));
totflowGel =  sum(flows([MesoTOgel,MacroTOgel,VMMacroTOgel]));
