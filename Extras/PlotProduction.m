...
%%primary and secondary production. Requires PlotIndirect.m and PlotTL 
%%to be run first
...
for Cycle = 1:5 %[1:5]
    ...
    PicoProp=Prod(:,1)./sum(Prod(:,1:3),2); %Prop of production by pico
    ...
    %Fish Production normalized to NPP
    ymax=10;
    ymin=10^-4;
    figure(f11)
    for i=1:width(EE)
    ...
    ylim([ymin ymax]);
    set(gca,'FontSize',20)
    ylabel('Production Normalized to NPP')
    xline([5.5 10.5 15.5],'--')
