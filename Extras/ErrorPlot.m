...
for Cycle=1:5
    ...
    if Cycle ==1
        ...
        scatter(xmean(20:36),ymean(20:36),sz,col(Cycle,:),'filled') %you can't really distinguish the squares at journal size
    else
        xmean=Inputs(:,Cycle*2-1);
        xmin=Inputs(:,Cycle*2);
        xminmod=xmin;
        ...
        scatter(xmean(21:39),ymean(21:39),sz,col(Cycle,:),"filled")
    end
    ...
    set(gca,'XScale','log')
    pbaspect([1 1 1])
    xticks([10^-8 10^-7 10^-6 10^-5 10^-4 10^-3 10^-2 10^-1 10^0 10^1 10^2])
